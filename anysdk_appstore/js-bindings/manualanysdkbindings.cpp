...
static bool jsb_anysdk_framework_AgentManager_getYAPPlugin(JSContext *cx, uint32_t argc, jsval *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    ...
    for (auto iter = plugins->begin(); iter != plugins->end(); ++iter)
    {
        ...
        JS::RootedObject paramObj(cx, js_get_or_create_jsobject<ProtocolYAP>(cx, yap_plugin));
...
public:
    ...
    static ProtocolYAPResultListener* getListenerByKey(std::string key)
    {
        auto listener = std_map[key];
        if (listener == NULL) {
            listener = new ProtocolYAPResultListener();
...
static bool jsb_anysdk_framework_ProtocolYAP_setResultListener(JSContext *cx, uint32_t argc, jsval *vp)
{
    CCLOG("in ProtocolAds_setAdsListener, argc:%d.", argc);
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    ...
    if (ProtocolYAPResultListener::std_map[p_id] == NULL)
    {
        ...
        ProtocolYAPResultListener* listener = ProtocolYAPResultListener::getListenerByKey(p_id);
        listener->setJSCallbackFunc( cx, args.get(0) );
        listener->setJSCallbackThis( cx, args.get(1) );
        listener->setJSCallbackCtx( cx, obj );
	    cobj->setResultListener(listener);
...
static bool jsb_anysdk_framework_ProtocolYAP_removeListener(JSContext *cx, uint32_t argc, jsval *vp)
{
...
static bool jsb_anysdk_framework_ProtocolYAP_yapForProduct(JSContext *cx, uint32_t argc, jsval *vp)
{
    ...
    if (argc != 1)
    {
		JS_ReportError(cx, "jsb_anysdk_framework_ProtocolYAP_yapForProduct : wrong number of arguments: %d, was expecting %d", argc, 0);
    	return true;
    }
    JS::RootedValue arg0(cx, args.get(0));
    if ( arg0.isObject() ){
        ...
        TProductInfo arg;
        bool ok = jsval_to_TYapmentInfo(cx, arg0, &arg);
