...
bool js_cocos2dx_extension_Manifest_getEngineVersion(JSContext *cx, uint32_t argc, JS::Value *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true; CC_UNUSED_PARAM(ok);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(cx, obj);
    cocos2d::extension::Manifest* cobj = (cocos2d::extension::Manifest *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_extension_Manifest_getEngineVersion : Invalid Native Object");
    if (argc == 0) {
        const std::string& ret = cobj->getEngineVersion();
        JS::RootedValue jsret(cx);
        ok &= std_string_to_jsval(cx, ret, &jsret);
        JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_extension_Manifest_getEngineVersion : error parsing return value");
        args.rval().set(jsret);
        return true;
    }

    JS_ReportErrorUTF8(cx, "js_cocos2dx_extension_Manifest_getEngineVersion : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}
...
bool js_cocos2dx_extension_Manifest_getMarketURL(JSContext *cx, uint32_t argc, JS::Value *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true; CC_UNUSED_PARAM(ok);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    ...
    if (argc == 0) {
        ...
        JS::RootedValue jsret(cx);
        ok &= std_string_to_jsval(cx, ret, &jsret);
        JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_extension_Manifest_getMarketURL : error parsing return value");
        args.rval().set(jsret);
        return true;
    }

    JS_ReportErrorUTF8(cx, "js_cocos2dx_extension_Manifest_getMarketURL : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}
...
void js_register_cocos2dx_extension_Manifest(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("getEngineVersion", js_cocos2dx_extension_Manifest_getEngineVersion, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
    ...
    JS::RootedObject proto(cx, JS_InitClass(
        ...
        dummy_constructor<cocos2d::extension::Manifest>, 0,
        nullptr,
        funcs,
        nullptr,
        nullptr));
...
bool js_cocos2dx_extension_AssetsManagerEx_constructor(JSContext *cx, uint32_t argc, JS::Value *vp)
{
    ...
    JS::RootedObject jsobj(cx);
    JS::RootedObject proto(cx, jsb_cocos2d_extension_AssetsManagerEx_prototype->get());
    jsb_ref_create_jsobject(cx, cobj, jsb_cocos2d_extension_AssetsManagerEx_class, proto, &jsobj, "cocos2d::extension::AssetsManagerEx");
    JS::RootedValue retVal(cx, JS::ObjectOrNullValue(jsobj));
    args.rval().set(retVal);
    if (JS_HasProperty(cx, jsobj, "_ctor", &ok) && ok) 
    {
...
void js_register_cocos2dx_extension_AssetsManagerEx(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec st_funcs[] = {
        JS_FN("create", js_cocos2dx_extension_AssetsManagerEx_create, 3, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
    };
